const i18n = {
    'es': {
        // Nav
        'nav-carta': 'Carta',
        'nav-sobre': 'Sobre TorreYan',
        'nav-entorno': 'El Entorno',
        'nav-eventos': 'Eventos',
        'btn-reservar': 'RESERVAR',

        // Hero
        'hero-descubre': '¡Descubre el<br>nuevo TorreYan',
        'hero-sub': 'Una cocina tradicional china en constante evolución, en el corazón de Zaragoza.',

        // About
        'about-title1': 'Reinventando la',
        'about-title2': 'cocina oriental',
        'about-p1': 'Nacido en la rica herencia culinaria de Asia, TorreYan llega a Zaragoza para redefinir el concepto de alta gastronomía china. Crecimos influenciados por la diversidad agrícola de nuestra tierra.',
        'about-p2': 'Desde pequeños, empezamos a cocinar usando ingredientes locales, desarrollando una pasión por la experimentación culinaria, pero manteniendo intacta el alma de la receta original.',
        'about-p3': 'Valoramos la riqueza de la gastronomía asiática y, junto a las técnicas modernas, buscamos mostrar su versatilidad y potencial infinito para la innovación.',
        'about-welcome': '¡Bienvenido a las cocinas de TorreYan!',

        // Tradition
        'trad-title1': 'Tradición',
        'trad-title2': 'milenaria.',
        'trad-title3': 'Técnicas',
        'trad-title4': 'modernas.',
        'trad-p': 'Nuestro compromiso es ofrecer una experiencia culinaria inolvidable, fusionando los sabores asiáticos más auténticos con un toque de innovación, en un ambiente que respira exclusividad.',
        'trad-btn': 'DESCUBRE LA CARTA',

        // Environment (New)
        'env-title1': 'Nuestro',
        'env-title2': 'Entorno',
        'env-sub': 'Un espacio diseñado para la elegancia, la privacidad y un confort absoluto.',

        // Footer Simple text
        'footer-address1': 'Centro Comercial Torre Outlet',
        'footer-address2': 'Av. de Logroño, km 6',
        'footer-address3': '50011 Zaragoza, España',
        'footer-contact-title': 'Contacto',
        'footer-work-with-us': 'Trabaja con nosotros',

        'footer-hours-title': 'Horario',
        'footer-hours-days': 'Lunes - Domingo',
        'footer-hours-lunch': '13:00 - 16:30',
        'footer-hours-dinner': '20:00 - 23:30',
        'footer-parking': 'Aparcamiento gratuito.',

        'footer-legal-title': 'Legal',
        'footer-legal-notice': 'Aviso Legal',
        'footer-privacy': 'Privacidad',
        'footer-cookies': 'Cookies',
        'footer-terms': 'Términos',

        'footer-copyright': '&copy; 2026 TorreYan 华宴. Todos los derechos reservados.',
        'footer-design': 'Diseño inspirado en Mítiko',

        // Menu Page (Carta)
        'nav-inicio': 'Inicio',
        'nav-volver': 'VOLVER',
        'carta-title': 'Nuestra Carta',
        'carta-entrantes': 'Entrantes',
        'carta-recomendaciones': 'Recomendaciones',
        'carta-mariscos': 'Mariscos',
        'carta-carnes': 'Carnes',
        'carta-verduras': 'Verduras',

        // Dishes Category 1: Entrantes
        'dish-es-1-name': 'Ensalada de Okra y Oreja de Madera',
        'dish-es-1-desc': 'Fresco y crujiente, aliñado con vinagreta suave.',
        'dish-es-2-name': 'Ensalada de Medusa',
        'dish-es-2-desc': 'Medusa marinada con pepino y toques de sésamo.',
        'dish-es-3-name': 'Ternera y Callos con Salsa Picante',
        'dish-es-3-desc': 'Famoso "Fu Qi Fei Pian" al estilo Sichuan.',
        'dish-es-4-name': 'Pollo al Estilo Sichuan con Salsa Picante',
        'dish-es-4-desc': 'Pollo frío bañado en salsa de aceite de chile y cacahuete.',
        'dish-es-5-name': 'Panceta de Cerdo con Salsa de Ajo',
        'dish-es-5-desc': 'Finas láminas de cerdo cocido servidas con salsa de ajo intenso.',
        'dish-es-6-name': 'Gambas Zafiro',
        'dish-es-6-desc': 'Crujientes y doradas, servidas con una salsa especial dulce y picante.',

        // Dishes Category 2: Recomendaciones
        'dish-re-1-name': 'Pescado Asado Especialidad de la Casa',
        'dish-re-1-desc': 'Pescado entero asado y estofado con verduras y especias.',
        'dish-re-2-name': 'Pescado Asado Picante',
        'dish-re-2-desc': 'Intenso sabor picante y adormecedor típico de Chongqing.',
        'dish-re-3-name': 'Pescado Asado al Ajo',
        'dish-re-3-desc': 'Versión suave y muy aromática con abundante ajo asado.',
        'dish-re-4-name': 'Estofado Picante Mao Xue Wang',
        'dish-re-4-desc': 'Sangre de pato y tripas estofadas en un caldo volcánico de chile y pimienta.',
        'dish-re-5-name': 'Cerdo Hervido en Caldo Picante',
        'dish-re-5-desc': 'Trozos tiernos de cerdo cocidos en un caldo rico en chiles.',
        'dish-re-6-name': 'Ternera Hervida en Caldo Picante',
        'dish-re-6-desc': 'Láminas de ternera jugosa en aceite picante y especias variadas.'
    },
    'en': {
        // Nav
        'nav-carta': 'Menu',
        'nav-sobre': 'About TorreYan',
        'nav-entorno': 'The Environment',
        'nav-eventos': 'Events',
        'btn-reservar': 'RESERVE',

        // Hero
        'hero-descubre': 'Discover the<br>new TorreYan',
        'hero-sub': 'A traditional Chinese cuisine in constant evolution, in the heart of Zaragoza.',

        // About
        'about-title1': 'Reinventing',
        'about-title2': 'oriental cuisine',
        'about-p1': 'Born in the rich culinary heritage of Asia, TorreYan arrives in Zaragoza to redefine the concept of haute Chinese gastronomy. We grew up influenced by the agricultural diversity of our land.',
        'about-p2': 'Since we were little, we started cooking using local ingredients, developing a passion for culinary experimentation, but keeping the soul of the original recipe intact.',
        'about-p3': 'We value the richness of Asian gastronomy and, together with modern techniques, we seek to show its versatility and infinite potential for innovation.',
        'about-welcome': 'Welcome to the kitchens of TorreYan!',

        // Tradition
        'trad-title1': 'Millenary',
        'trad-title2': 'tradition.',
        'trad-title3': 'Modern',
        'trad-title4': 'techniques.',
        'trad-p': 'Our commitment is to offer an unforgettable culinary experience, fusing the most authentic Asian flavors with a touch of innovation, in an environment that breathes exclusivity.',
        'trad-btn': 'DISCOVER THE MENU',

        // Environment (New)
        'env-title1': 'Our',
        'env-title2': 'Environment',
        'env-sub': 'A space designed for elegance, privacy, and absolute comfort.',

        // Footer Simple text
        'footer-address1': 'Torre Outlet Shopping Center',
        'footer-address2': 'Av. de Logroño, km 6',
        'footer-address3': '50011 Zaragoza, Spain',
        'footer-contact-title': 'Contact',
        'footer-work-with-us': 'Work with us',

        'footer-hours-title': 'Opening Hours',
        'footer-hours-days': 'Monday - Sunday',
        'footer-hours-lunch': '1:00 PM - 4:30 PM',
        'footer-hours-dinner': '8:00 PM - 11:30 PM',
        'footer-parking': 'Free parking.',

        'footer-legal-title': 'Legal',
        'footer-legal-notice': 'Legal Notice',
        'footer-privacy': 'Privacy',
        'footer-cookies': 'Cookies',
        'footer-terms': 'Terms',

        'footer-copyright': '&copy; 2026 TorreYan 华宴. All rights reserved.',
        'footer-design': 'Design inspired by Mítiko',

        // Menu Page (Carta)
        'nav-inicio': 'Home',
        'nav-volver': 'BACK',
        'carta-title': 'Our Menu',
        'carta-entrantes': 'Starters',
        'carta-recomendaciones': 'Chef\'s Recommendations',
        'carta-mariscos': 'Seafood',
        'carta-carnes': 'Meats',
        'carta-verduras': 'Vegetables',

        // Dishes - Starters
        'dish-es-1-name': 'Okra and Wood Ear Mushroom Salad',
        'dish-es-1-desc': 'Fresh and crispy, dressed with a mild vinaigrette.',
        'dish-es-2-name': 'Jellyfish Salad',
        'dish-es-2-desc': 'Marinated jellyfish with cucumber and sesame hints.',
        'dish-es-3-name': 'Beef and Tripe in Chili Sauce',
        'dish-es-3-desc': 'Famous "Fu Qi Fei Pian" Sichuan style.',
        'dish-es-4-name': 'Mouthwatering Sichuan Chicken',
        'dish-es-4-desc': 'Cold cooked chicken bathed in chili oil and peanut sauce.',
        'dish-es-5-name': 'Pork Belly with Garlic Sauce',
        'dish-es-5-desc': 'Thin slices of cooked pork served with intense garlic sauce.',
        'dish-es-6-name': 'Sapphire Prawns',
        'dish-es-6-desc': 'Crispy golden prawns served with a special sweet and spicy sauce.',

        // Dishes - Recs
        'dish-re-1-name': 'House Special Grilled Fish',
        'dish-re-1-desc': 'Whole grilled fish stewed with vegetables and spices.',
        'dish-re-2-name': 'Spicy Grilled Fish',
        'dish-re-2-desc': 'Intensely spicy and numbing, typical of Chongqing.',
        'dish-re-3-name': 'Garlic Grilled Fish',
        'dish-re-3-desc': 'Mild and highly aromatic version with abundant roasted garlic.',
        'dish-re-4-name': 'Mao Xue Wang Spicy Stew',
        'dish-re-4-desc': 'Duck blood and tripe stewed in a volcanic chili broth.',
        'dish-re-5-name': 'Boiled Pork in Spicy Broth',
        'dish-re-5-desc': 'Tender pork slices cooked in a rich chili broth.',
        'dish-re-6-name': 'Boiled Beef in Spicy Broth',
        'dish-re-6-desc': 'Juicy beef slices in chili oil and mixed spices.'
    },
    'zh': {
        // Nav
        'nav-carta': '菜单',
        'nav-sobre': '关于华宴',
        'nav-entorno': '用餐环境',
        'nav-eventos': '活动',
        'btn-reservar': '预定',

        // Hero
        'hero-descubre': '探索全新的<br>TorreYan',
        'hero-sub': '位于萨拉戈萨中心，不断演进的传统中国美食。',

        // About
        'about-title1': '重塑',
        'about-title2': '东方美食',
        'about-p1': '诞生于亚洲丰富的烹饪传统中，华宴来到萨拉戈萨，重新定义高级中文字段的概念。我们在家乡农业多样性的影响下成长。',
        'about-p2': '从小我们就开始使用当地食材烹饪，培养了对烹饪实验的极大热情，但始终保持原始配方的灵魂完好无损。',
        'about-p3': '我们重视亚洲美食的丰富性，并结合现代技术，力求展现其多功能性和无限的创新潜力。',
        'about-welcome': '欢迎来到华宴的厨房！',

        // Tradition
        'trad-title1': '千年',
        'trad-title2': '传统',
        'trad-title3': '现代',
        'trad-title4': '技术',
        'trad-p': '我们的承诺是提供令人难忘的烹饪体验，将最正宗的亚洲风味与创新融为一体，置身于充满专属感的环境中。',
        'trad-btn': '查看完整菜单',

        // Environment (New)
        'env-title1': '我们的',
        'env-title2': '环境',
        'env-sub': '专为优雅、私密和绝对舒适而设计的空间。',

        // Footer Simple text
        'footer-address1': 'Torre Outlet 购物中心',
        'footer-address2': 'Av. de Logroño, 6公里处',
        'footer-address3': '50011 萨拉戈萨, 西班牙',
        'footer-contact-title': '联系方式',
        'footer-work-with-us': '加入我们',

        'footer-hours-title': '营业时间',
        'footer-hours-days': '周一 至 周日',
        'footer-hours-lunch': '13:00 - 16:30',
        'footer-hours-dinner': '20:00 - 23:30',
        'footer-parking': '免费停车。',

        'footer-legal-title': '法律条款',
        'footer-legal-notice': '法律声明',
        'footer-privacy': '隐私条款',
        'footer-cookies': 'Cookie政策',
        'footer-terms': '服务条款',

        'footer-copyright': '&copy; 2026 TorreYan 华宴. 保留所有权利。',
        'footer-design': '设计灵感来自 Mítiko',

        // Menu Page (Carta)
        'nav-inicio': '首页',
        'nav-volver': '返回',
        'carta-title': '我们的菜单',
        'carta-entrantes': '前菜',
        'carta-recomendaciones': '主推菜品',
        'carta-mariscos': '海鲜类',
        'carta-carnes': '肉类',
        'carta-verduras': '蔬菜',

        // Dishes - Starters
        'dish-es-1-name': '秋葵拌木耳',
        'dish-es-1-desc': '清新爽脆，特调油醋汁拌匀即可。',
        'dish-es-2-name': '葱油海蜇皮',
        'dish-es-2-desc': '劲道海蜇皮搭配黄瓜丝，葱油提香。',
        'dish-es-3-name': '夫妻肺片',
        'dish-es-3-desc': '川菜经典，麻辣鲜香，回味悠长。',
        'dish-es-4-name': '口水鸡',
        'dish-es-4-desc': '鲜嫩鸡肉浸泡在秘制红油中，口水直流。',
        'dish-es-5-name': '蒜泥白肉',
        'dish-es-5-desc': '肥而不腻的白肉片，浇上浓郁蒜泥汁。',
        'dish-es-6-name': '蓝宝石虾',
        'dish-es-6-desc': '外酥里嫩的炸虾球，搭配特制酸甜酱。',

        // Dishes - Recs
        'dish-re-1-name': '招牌烤鱼',
        'dish-re-1-desc': '整条鲜鱼炭火烤制，配以精选时蔬炖煮，香味四溢。',
        'dish-re-2-name': '香辣烤鱼',
        'dish-re-2-desc': '重庆地道风味，麻辣刺激，越吃越过瘾。',
        'dish-re-3-name': '蒜香烤鱼',
        'dish-re-3-desc': '浓郁蒜蓉铺满鱼身，不辣星人的最爱。',
        'dish-re-4-name': '毛血旺',
        'dish-re-4-desc': '鸭血、毛肚等丰富食材在那一抹红油中翻滚。',
        'dish-re-5-name': '水煮肉片',
        'dish-re-5-desc': '滑嫩肉片在麻辣高汤中汆烫，鲜香扑鼻。',
        'dish-re-6-name': '水煮牛肉',
        'dish-re-6-desc': '浓香牛肉片，经典的川味做法，辣而不燥。'
    }
};

window.siteTranslations = i18n;
